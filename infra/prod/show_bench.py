...
hdr = ("bucket", "out", "queue", "prefill", "decode", "itl", "total")
print("%-8s%5s%8s%8s%8s%9s%8s" % hdr)
for r in rows:
    print("%-8s%5d%8.2f%8.2f%8.2f%9.4f%8.2f" % (
        r["bucket"], r["output_tokens"], r["queue_s"], r["prefill_s"],
        r["decode_s"], r["itl_s"], r["total_s"]))
...
print("headline   throughput %.2f  per_stream %.3f  occupancy %.3f" % (
    h["throughput_tps"], h["per_stream_tps"], h["occupancy"]))
print("context    %d tokens in %.1f s wall" % (c["output_tokens"], c["wall_s"]))
